 ...
 * @implements [SRS-EP-32] empty tap paste chrome only while idle
 */
...
public:
    ...
    void onTap(const PointerSample &s) override
    {
        ...
        if (!id.empty()) {
            m_caps->selection->setPasteOrigin(s.panel.x(), s.panel.y(), w.x, w.y);
            m_caps->selection->setIds({id});
            m_caps->selection->setPhase(SelectionPhase::Selected);
            m_didMutate = true;
        } else if (m_caps->selection->ids().empty()) {
            m_caps->selection->setPasteOrigin(s.panel.x(), s.panel.y(), w.x, w.y);
            m_didMutate = true;
        } else {
