...
/**
 * MarqueeOperation — RawPointer marquee select under SelectionMode.
...
public:
    explicit MarqueeOperation(SelectionStrokeHost host)
    ...
    void onDown(const PointerSample &s) override
    {
        if (!m_host.session || !m_host.applyIntent)
            return;
        m_host.applyIntent(m_host.session->beginMarqueeOrLasso(s.panel.x(), s.panel.y(), false));
    }
    ...
    void onMove(const PointerSample &s) override
    {
        if (!m_host.session || !m_host.applyIntent)
            return;
        m_host.applyIntent(m_host.session->updateMarquee(s.panel.x(), s.panel.y()));
    }
