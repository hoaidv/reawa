...
TabletWindow {
    ...
    Rectangle {
        id: encloseCta
        z: 21
        visible: toolCanvas.encloseVisible
        x: toolCanvas.encloseCtaRect.x
        y: toolCanvas.encloseCtaRect.y
        width: 64
        height: 64
        color: "white"
        border.color: "black"
        ...
        TapHandler {
            ...
            grabPermissions: PointerHandler.CanTakeOverFromItems
                             | PointerHandler.ApprovesCancellation
            onTapped: toolCanvas.encloseSelection()
        }
    }
    ...
    Repeater {
        id: selectHandles
        z: 21
        model: toolCanvas.handleCount
        delegate: ResizeKnob {
            readonly property var _r: toolCanvas.selectionBoundsRect
            readonly property real _h: toolCanvas.handleSize
            readonly property real _hit: Math.max(_h, 64)
            readonly property point _pt: {
                var r = _r
                var n = toolCanvas.handleCount
                var i = index
                if (n === 8) {
                    var p8 = [
                        Qt.point(r.x, r.y),
            ...
            x: _pt.x - _hit / 2
            y: _pt.y - _hit / 2
            visible: toolCanvas.handleCount > 0 && _r.width > 0 && _r.height > 0
        }
    }
    ...
    Rectangle {
        id: inkScaleChip
        z: 21
        visible: toolCanvas.modeChipVisible
        x: toolCanvas.modeChipRect.x
        y: toolCanvas.modeChipRect.y
        width: Math.max(120, toolCanvas.modeChipRect.width)
        height: Math.max(36, toolCanvas.modeChipRect.height)
        color: "white"
        border.color: "black"
        border.width: 2
        Text {
        ...
        TapHandler {
            ...
            grabPermissions: PointerHandler.CanTakeOverFromItems
                             | PointerHandler.ApprovesCancellation
            onTapped: toolCanvas.tapModeChip()
        }
    }

    Text {
        z: 21
        visible: toolCanvas.encloseRefuseReason.length > 0
        x: encloseCta.visible ? encloseCta.x : 24
        y: encloseCta.visible ? encloseCta.y + encloseCta.height + 6 : 80
        font.pixelSize: 16
        color: "black"
        text: toolCanvas.encloseRefuseReason
    }
    ...
    Rectangle {
        id: manipUnavailable
        z: 21
        visible: toolCanvas.manipulationUnavailable.length > 0
        x: toolCanvas.manipulationUnavailableRect.x
        y: toolCanvas.manipulationUnavailableRect.y
        width: Math.max(220, toolCanvas.manipulationUnavailableRect.width)
        height: Math.max(36, toolCanvas.manipulationUnavailableRect.height)
        color: "white"
        border.color: "black"
        border.width: 2
        Text {
            ...
            font.pixelSize: 14
            color: "black"
            text: toolCanvas.manipulationUnavailable
        }
    }
