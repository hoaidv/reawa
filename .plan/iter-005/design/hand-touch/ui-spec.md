...
scenes:
  ...
  - hand-touch-finger-anchor-noop.html
  ...
  - hand-touch-pen-resize-after-finger-select.html
...
- Compose: UI-EP-04 ToolChip (3+2+Undo/Redo); UI-EP-02 overlay (double-rail + 28/56 du handles)
...
| Target frames | Landscape tablet **1872×1404** (native 1404×1872 @ 226 dpi). Not phone chrome |
| Responsive strategy | per-target — one panel size; no reflow |
| Breakpoints / resize | N/A — fixed panel |
| Safe areas / fixed regions | ToolChip floats orientation-top; overlay is content-space; world tracks viewport |
| Input model | **Pen** for content and &lt;64 du handles; **finger** for ≥64 du (box AABB, ToolChip 64 du, Enclose 64 du). Two fingers = viewport. No keyboard |
| Nav paradigm | In-scene states on one surface — no push / sheet / modal |
| Target minimum | Finger-eligible **64×64 du**; resize anchors visual **28** / hit **56 du** (pen-only) |
| Density | compact 1-bit |
| Hover | **N/A** — no hover, no focus, no cursor, no motion |
| Preview | Navigator `data-preview-scale="mobile"` @ **80%** of 1872×1404 (campaign lock). Scale is navigator chrome only |
...
flowchart LR
  ...
  hit -->|finger on 56 du handle| noop[hand.finger_anchor_noop]
  hit -->|pen on handle| resize[hand.pen_resize_after_finger_select]
...
| SelectionOverlay | Bounds + 8 resize handles (no rotation) + mode toggle. Content-space | SelectionOverlay | hidden / selected / moving / resizing / noop-handle |
...
|---|---|---|---|---|
| ToolChip | **reuse** UI-EP-04 | `.c-tool-chip` | 3 clusters, 64×64, gap 32 | ToolChip |
...
| SelectionOverlay | **reuse** UI-EP-02 | `.c-bounds` + `.c-handle` | selected, moving, resizing, handle noop | SelectionOverlay |
| InkScaleModeToggle | **reuse** UI-EP-02 | `.c-mode-toggle` | withBounds; **pen-only** (36 du &lt; 64) | SelectionOverlay |
| FingerContact | **build** | `.c-finger` | one / two / ignored (dashed) | GestureAnnotate |
| AnchorNoopIndicator | **build** | `.c-indicator` `#ind.finger_anchor_noop` | hidden / visible (1-bit hatch; no blink animation) | SelectionOverlay |
| PenTipAnnotate | **build** | `.c-pen-tip` | default | GestureAnnotate |

No pan-mode tool. No hand-tool tile. No finger-resize handles. No properties panel. No rotation handle. No ghost / marquee stand-in for move.
...
| `hand.finger_anchor_noop` | Finger on &lt;64 du handle | Handle hatch (`ind.finger_anchor_noop`); **0** transform; pen-only 56 du callout | CHL-0019 size rule |
...
| `hand.pen_resize_after_finger_select` | Pen on 56 du anchor after finger select | Resize allowed; ink scales (`withBounds`); `sel_freeform` stays | REQ-10 mixed path |
...
| Resize handle (56 du, pen-only) | N/A | N/A | ✓ invert (pen) | finger → noop hatch, 0 transform | — | — | — | hand-touch-states.html |
| Mode toggle (36 du, pen-only) | N/A | N/A | ✓ invert | — | — | current mode label | — | hand-touch-states.html |
...
| `ind.finger_anchor_noop` | — | — | — | — | — | visible hatch chip | — | hand-touch-states.html |
...
- Targets: chip **64×64 du**; handles hit **56 du** (pen); finger floor **64 du**
...
|---|---|---|
| `ind.finger_anchor_noop` | Finger: 0 transform — pen only (56 du) | device unit |
...
| Handle / finger_anchor_noop | SRS-EP-21/22 | 0 resize from finger; pen still resizes | 56 du &lt; 64 |
...
| Handle / pen_resize_after_finger_select | SRS-EP-21 + SRS-EP-11 | pen on same anchor resizes | After finger select |
...
| Hit: resize / 6 square anchors 28/56 pen-only | 8 SmartGroup resize handles @ 56 hit (reuse SRS-EP-12 overlay; 6 anchors are `ovl.select_anchors` for node-union, not this box path) | match (EP-12 reuse) |
| Additive: `ind.finger_anchor_noop` | Hatch chip + handle `.is-noop` | match |
...
| No finger-resize handles | Handles stay 56 du; finger → noop | match |
| States matrix 9 ids | 9 scene files 1:1 | match |
...
| `hand.finger_anchor_noop` | `hand-touch-finger-anchor-noop.html` | no | |
...
| `hand.pen_resize_after_finger_select` | `hand-touch-pen-resize-after-finger-select.html` | no | |
...
| `hand.finger_hit_box` | SE handle (finger) | in-scene | `hand.finger_anchor_noop` |
| `hand.finger_hit_box` | caption → pen resize | in-scene | `hand.pen_resize_after_finger_select` |
...
| `hand.finger_anchor_noop` | lift | in-scene | `hand.finger_hit_box` |
| `hand.pen_resize_after_finger_select` | release | in-scene | `hand.finger_hit_box` |
...
| Pen tip (annotate) | system | `../system/assets/icon-hand-touch-pen-tip.svg` | pen_resize |
| Pen-only | system | `../system/assets/icon-hand-touch-pen-only.svg` | anchor noop / states |
| No-op | system | `../system/assets/icon-hand-touch-noop.svg` | `ind.finger_anchor_noop` |
...
| SelectionOverlay | screen | `./components/selection-overlay.html` | selected; moving; handle noop; handle pressed |
| FingerContact | screen | `./components/finger-contact.html` | one; two; ignored dashed |
| AnchorNoopIndicator | screen | `./components/anchor-noop-indicator.html` | visible hatch |
...
| Accessibility audit | 1-bit contrast; 64 du finger; 56 du pen handles; labels; no color-only |
