...
| **NodeEmphasis** | Recog blink, membership stroke-stamp, AABB highlight (ToolCanvas Mono, partial damage) | Selection knobs; document FullClear; inlined into ToolCanvasItem::paint |
...
- `NodeEmphasis` is **host-owned** ([CHL-0030](../../../.plan/iter-005/challenges/CHL-0030-node-emphasis.md)). Every Mode paints it (under in-flight Operation overlay). TabletCanvas does not bake blink/highlight into `m_image`.
